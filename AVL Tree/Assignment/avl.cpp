...
void avl::preorderTraversal(node* tempNode){
  if (tempNode == NULL)
  {
    return;
  }
  cout<<"Node: "<<tempNode->getKey()<<" Parent: ";
  if(tempNode->getParent())
    cout<<tempNode->getParent()->getKey()<<endl;
  else
    cout<<"NULL"<<endl;
  ...
  if(tempNode->getLeft())
    ...
    cout<<"NULL";
  cout<<" Nodes in Left subtree: "<<tempNode->getlnum();
  ...
  if(tempNode->getRight())
    ...
    cout<<"NULL";
  cout<<" Nodes in Right subtree: "<<tempNode->getrnum();
  cout<<endl<<endl;
  preorderTraversal(tempNode->getLeft());
  preorderTraversal(tempNode->getRight());
}
...
int avl::updateNodeNumbers(node *n){
  int num=0;
  if(n!=NULL){
    int l_num = updateNodeNumbers(n->getLeft());
    int r_num = updateNodeNumbers(n->getRight());
    int total_num = l_num+r_num;
    num=total_num+1;
    n->setlnum(l_num);
    n->setrnum(r_num);
  }
